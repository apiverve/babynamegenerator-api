...
class BabynamegeneratorValidationException implements Exception {
  final List<String> errors;

  BabynamegeneratorValidationException(this.errors);

  @override
  String toString() => 'BabynamegeneratorValidationException: ${errors.join("; ")}';
}
...
/// Baby Name Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/babynamegenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [gender] (required) - The gender for the generated names
/// * [count] - The number of names to generate [min: 1, max: 100]
class BabynamegeneratorClient {
  final String apiKey;
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BabynamegeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Baby Name Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<BabynamegeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BabynamegeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BabynamegeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BabynamegeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BabynamegeneratorException('Resource not found');
      } else {
        throw BabynamegeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BabynamegeneratorException) rethrow;
      throw BabynamegeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Baby Name Generator API client.
class BabynamegeneratorException implements Exception {
  final String message;

  BabynamegeneratorException(this.message);

  @override
  String toString() => 'BabynamegeneratorException: $message';
}
